...
straighttalk_write_binary_latin1_with_special_char_directly(_) ->
	...
	<<"Hello W?rld!">> = String, 
    % in /= out:
    % The Latin-1 database got a Latin-1 encoded ø but had been told
    % it would receive UTF-8 characters. In that context the Latin-1 ø
    % was an invalid bit pattern and was replaced by '?'.
...
worker_insert_as_latin1_binary(V) ->
    ...
    try 
        unicode:characters_to_list(Value,utf8), % catch case trigger
       	worker_insert_via_statement(Value, latin1, list_to_binary(castrate(V)))
    catch
    ...
    % The Latin-1 database gets Latin-1 encoded characters but had been told
    % it would receive UTF-8 characters. In that context the Latin-1 characters
    % > 128 are invalid bit patterns and are replaced by '?' (castrate/1).
